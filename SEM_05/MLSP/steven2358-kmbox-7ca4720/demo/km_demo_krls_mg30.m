...
pars.kernel.type = 'gauss';	% kernel type
		% kernel parameter (width in case of Gaussian kernel)
ERR = zeros(500,2);
EST = zeros(100,2);
...
for k = 1:2
    ...
    pars.M = dict(k);				% maximum dictionary size
    pars.kernel.par = 1;	
...
legend({" dict size:10 ","dict size 100"});
...
legend({"Y_true"," dict size:10 ","dict size 100"});